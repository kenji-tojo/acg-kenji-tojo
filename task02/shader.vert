...
void main()
{
    ...
    z0 = z0-cam_z_pos;
    float radi = sqrt(x0*x0+y0*y0);
    float dist = sqrt(radi*radi+z0*z0);
    float theta = acos(z0/dist);

    float x1 = x0*theta/(radi*PI);
    float y1 = y0*theta/(radi*PI);
    float z1 = -1/(1+dist); // z1 should be inside [-1,1]
